...
// ── Roblox helpers ────────────────────────────────────────────────────────────
async function fetchRobloxPresence(userId) {
  if (!userId) return null;
  return apiFetch(`/api/users/${userId}/roblox/presence`);
}
const ROBLOX_ICON_SVG = `<svg class="platform-widget-icon" viewBox="0 0 24 24" width="13" height="13" fill="currentColor"><path d="M18.926 23.998 0 18.074 5.075.002 24 5.926z"/></svg>`;
function renderRobloxWidget(data) {
...
function renderNav(activePage) {
  ...
  root.innerHTML = `
    <nav class="nav">
      ...
      <div class="nav-right">
        <a href="watch.html" class="nav-new nav-watch-link" title="Watch Party" style="display:inline-flex;align-items:center;gap:5px;"><img src="images/emoticons/icecream.png" alt="" style="width:18px;height:18px;object-fit:contain;">watch</a>
