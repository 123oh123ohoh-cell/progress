...
function openPresenceSocket(activePage) {
  ...
  try {
    ...
    // Tells the server whether THIS tab is currently focused, so someone
    // can show as "idle" rather than fully "online" while their tab is
    // backgrounded (Discord calls this state "Idle"). Sent once right
    // after connecting (correcting the server's default "active" guess if
    // the tab actually started out hidden) and again on every subsequent
    // visibility change.
    const sendActivity = () => {
